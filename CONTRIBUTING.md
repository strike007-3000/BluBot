...
2.  **Staged Pipeline (v3.18.0)**: Core logic follows a linear, staged flow (**Curation → Synthesis → Broadcast → Persistence**). Always pass state via frozen dataclasses from `src/models.py`. The Telegram gateway supports an early-exit `/brief` path that bypasses the standard pipeline.
