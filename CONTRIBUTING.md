...
2.  **Staged Pipeline (v3.18.1)**: Core logic follows a linear, staged flow (**Curation → Synthesis → Broadcast → Persistence**). Always pass state via frozen dataclasses from `src/models.py`. The Telegram gateway supports an early-exit `/brief` path that bypasses the standard pipeline.
...
3.  **The "Merge" Requirement**: Your PR must pass all 93+ automated tests, the **CI Verification (`test`)**, and **CodeQL Analysis**.
4.  **Documentation**: If you change the persona, scoring engine, Telegram commands, or feed network, update [WIKI_MANUAL.md](docs/WIKI_MANUAL.md) and [README.md](README.md) to reflect the shift.
