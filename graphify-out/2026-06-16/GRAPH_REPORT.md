...
## Corpus Check
- 43 files · ~23,861 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 441 nodes · 699 edges · 58 communities (39 shown, 19 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 61 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2b4d95d0`
...
Cohesion: 0.11
Nodes (17): Determines if the current execution was manually triggered (Persona logic)., Determines if scheduling rest locks should be ignored (Infrastructure logic)., Validates critical settings and returns True if valid., Centralized, typed configuration for BluBot., Settings, Verify that validate() always returns True in dry run., Verify validate() fails when required production keys are missing., Verify validate() succeeds with valid parameters. (+9 more)

### Community 4 - "Feed configuration and dry-run diagnostics"
Cohesion: 0.06
Nodes (46): test_config_validation(), main(), test_full_dry_run(), test_scoring(), fetch_feed_headlines(), main(), Fetches first 5 headlines from a single RSS feed., get_version() (+38 more)

### Community 5 - "Feed Vanguard RSS audit resilience"
Cohesion: 0.14
Nodes (9): Feed Vanguard, Three-Tier State Persistence, Performs a comprehensive feed audit and displays a health report., run_standalone_audit(), diagnostic(), AsyncClient, Helper to check a single feed's health., Returns the list of feeds that are NOT currently blacklisted or have passed thei (+1 more)

### Community 6 - "Atomic file locking & state persistence"
Cohesion: 0.19
Nodes (9): test_persistence_resilience(), FileLock, _load_gist_state(), load_seen_articles(), Cross-platform advisory file lock context manager., Helper to pull state from a private GitHub Gist., 3-Tier Resilience: Local -> Backup -> Gist -> Default., 3-Tier Persistence: Atomic Write -> Backup Commit -> Remote Sync. (+1 more)
...
Cohesion: 0.22
Nodes (8): Image Compression & RGB Defense, load_session_string(), Helper to push state to a private GitHub Gist., Saves the BlueSky session string to a private file., Loads the cached BlueSky session string if it exists., _save_gist_state(), save_session_string(), SSRF Prevention
...
Cohesion: 0.22
Nodes (8): 1. 3-Tier State Resilience (The Fortress), 2. Humanization Patterning (The Narrator), 3. Feed Vanguard Automation, 4. Critical Bug Remediation, 📊 Documentation Updates, 🌟 Key Features (v3.8.0 - v3.8.2), PR Description: The Fortress & The Narrator (v3.8.2) 🏰🥁, 🧪 Verification & Stability

### Community 13 - "Dashboard telemetry status migration"
Cohesion: 0.67
Nodes (3): Automatically update the STATUS.md dashboard without blocking the event loop., update_status_dashboard(), test_dashboard()
...
## Knowledge Gaps
- **115 isolated node(s):** `Any`, `graphify`, `Workflow: graphify`, `🧠 The Sage Philosophy`, `🧱 Architectural Guardrails` (+110 more)
...
- **Why does `Settings` connect `Global settings & config generation` to `Logger & Secret Redaction Engine`, `Feed configuration and dry-run diagnostics`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Why does `VanguardManager` connect `Data Models & Pipeline Staging` to `Feed Vanguard RSS audit resilience`?**
  _High betweenness centrality (0.037) - this node is a cross-community bridge._
- **Why does `SafeLogger` connect `Logger & Secret Redaction Engine` to `Utils & SSRF prevention architecture`, `Global settings & config generation`, `Feed configuration and dry-run diagnostics`, `Atomic file locking & state persistence`?**
