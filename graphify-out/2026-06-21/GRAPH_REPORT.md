...
## Corpus Check
- 38 files · ~27,012 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 444 nodes · 771 edges · 51 communities (34 shown, 17 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 61 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4ade7346`
...
5. `send_draft_for_approval()` - 18 edges
6. `CurationResult` - 17 edges
7. `curation_stage()` - 16 edges
8. `Article` - 16 edges
9. `InteractionNote` - 16 edges
10. `SafeLogger` - 15 edges
...
Cohesion: 0.11
Nodes (42): article_matches_topic(), broadcast_stage(), curation_stage(), persistence_stage(), Any, AsyncClient, Stage 1: Fetch and Score Raw News., Stage 3: Multi-platform delivery. (+34 more)
...
Cohesion: 0.08
Nodes (31): Determines if the current execution was manually triggered (Persona logic)., Determines if scheduling rest locks should be ignored (Infrastructure logic)., Validates critical settings and returns True if valid., Centralized, typed configuration for BluBot., Settings, Any, AsyncClient, Sends the generated post draft and image to Telegram for approval.     Waits up (+23 more)

### Community 4 - "Feed configuration and dry-run diagnostics"
Cohesion: 0.06
Nodes (57): interaction_stage(), main(), Stage 2: AI Summarization and Persona Application., Handles social interactions (mentions/replies) with humanized engagement., Synchronous implementation of STATUS.md update to be offloaded to thread., Automatically update the STATUS.md dashboard without blocking the event loop., synthesis_stage(), update_status_dashboard() (+49 more)
...
Cohesion: 0.13
Nodes (12): FileLock, _load_gist_state(), load_seen_articles(), Cross-platform advisory file lock context manager., Helper to pull state from a private GitHub Gist., Helper to push state to a private GitHub Gist., Helper to save state to a JSON file., 3-Tier Resilience: Local -> Backup -> Gist -> Default. (+4 more)

### Community 7 - "Seen interactions & smart text truncation"
Cohesion: 0.23
Nodes (5): Image Compression & RGB Defense, fetch_feed_headlines(), main(), Fetches first 5 headlines from a single RSS feed., SSRF Prevention
...
Cohesion: 0.22
Nodes (8): 🔄 1. Interactive Text Curation Regeneration, 1. What was Deleted or Simplified, 🎨 2. Nvidia Image & Alt-Text Regeneration, 2. Why the Simpler Version is Safe, 3. Verification & Tests Run, 🛠️ Compliance with `AGENTS.md` Rules, 🚀 PR v3.13.0: Interactive Text & Image Regeneration from Telegram, Proposed Upgrades

### Community 13 - "Dashboard telemetry status migration"
Cohesion: 0.12
Nodes (21): main(), test_full_dry_run(), test_scoring(), get_version(), Legacy wrapper for the new Settings validation logic., Legacy wrapper for Gemini model self-discovery., validate_config(), validate_gemini_model_priority() (+13 more)
...
Cohesion: 0.18
Nodes (16): clean_hashtags_if_needed(), post_to_bluesky(), post_to_mastodon(), post_to_threads(), Posts to Mastodon with Conditional Multi-Post Threading (The Weaver)., Helper to strip or format hashtags based on platform settings., Posts to Threads with Conditional Multi-Post Threading (The Weaver)., Posts to Bluesky with Conditional Multi-Post Threading (The Weaver). (+8 more)
...
## Knowledge Gaps
- **96 isolated node(s):** `Any`, `AsyncClient`, `Any`, `graphify`, `Workflow: graphify` (+91 more)
...
- **Why does `Settings` connect `Global settings & config generation` to `Data Models & Pipeline Staging`, `Logger & Secret Redaction Engine`, `Feed configuration and dry-run diagnostics`, `Seen interactions & smart text truncation`?**
  _High betweenness centrality (0.072) - this node is a cross-community bridge._
- **Why does `VanguardManager` connect `Feed Vanguard RSS audit resilience` to `Data Models & Pipeline Staging`, `Feed configuration and dry-run diagnostics`, `Seen interactions & smart text truncation`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Why does `SafeLogger` connect `Logger & Secret Redaction Engine` to `Global settings & config generation`, `Dashboard telemetry status migration`, `Atomic file locking & state persistence`, `Seen interactions & smart text truncation`?**
