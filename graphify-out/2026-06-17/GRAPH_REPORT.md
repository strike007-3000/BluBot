...
## Corpus Check
- 37 files · ~24,786 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 434 nodes · 735 edges · 50 communities (34 shown, 16 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 61 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5aabcaeb`
...
1. `Settings` - 22 edges
2. `VanguardManager` - 21 edges
3. `synthesis_stage()` - 19 edges
4. `📖 BluBot Elite Sage: The Complete Manual` - 19 edges
5. `CurationResult` - 17 edges
6. `curation_stage()` - 16 edges
7. `Article` - 16 edges
8. `InteractionNote` - 16 edges
9. `SafeLogger` - 15 edges
10. `SynthesisResult` - 15 edges
...
Cohesion: 0.10
Nodes (49): article_matches_topic(), broadcast_stage(), curation_stage(), main(), persistence_stage(), Any, AsyncClient, Stage 1: Fetch and Score Raw News. (+41 more)

### Community 2 - "Logger & Secret Redaction Engine"
Cohesion: 0.08
Nodes (20): LogRecord, _HumanFormatter, _JsonFormatter, Any, Redacts secrets from all log content.     Uses both keyword-based patterns and, Formats log records as clean, colorized text for terminal readability., Structured, security-hardened logger with secret redaction and JSON output., Formats log records as single-line JSON strings. (+12 more)

### Community 3 - "Global settings & config generation"
Cohesion: 0.15
Nodes (16): interaction_stage(), Handles social interactions (mentions/replies) with humanized engagement., Synchronous implementation of STATUS.md update to be offloaded to thread., _update_status_dashboard_sync(), fetch_bluesky_mentions(), fetch_mastodon_mentions(), fetch_threads_replies(), Fetches recent mentions and replies from Bluesky within 24 hours. (+8 more)

### Community 4 - "Feed configuration and dry-run diagnostics"
Cohesion: 0.05
Nodes (45): calculate_relevance_score(), fetch_single_feed(), generate_image_alt_text(), generate_mentor_insight(), generate_nvidia_image(), generate_visual_prompt(), prune_gemini_model_priority_async(), Determines if a model supports the thinking_budget parameter.     Currently sup (+37 more)
...
Cohesion: 0.11
Nodes (18): Image Compression & RGB Defense, get_with_safe_redirects(), load_session_string(), Helper to push state to a private GitHub Gist., Helper to save state to a JSON file., 3-Tier Persistence: Atomic Write -> Backup Commit -> Remote Sync., Resolves a hostname and returns only public IP candidates., Temporarily constrains DNS resolution for one hostname to a prevalidated set. (+10 more)

### Community 8 - "Utils & SSRF prevention architecture"
Cohesion: 0.22
Nodes (11): _load_gist_state(), load_json_state(), load_seen_articles(), load_seen_interactions(), Helper to pull state from a private GitHub Gist., Helper to load JSON data from a file path., Loads the list of social interaction IDs we've already responded to., Saves the list of social interaction IDs to persistent store. (+3 more)
...
Cohesion: 0.12
Nodes (20): get_image_mime(), _is_public_ip(), Checks if an IP address is a routable public address., Detects MIME type from image bytes for broadcaster fidelity., Unicode-aware byte-level truncation to prevent Bluesky index errors., Truncates text at word boundaries within the limit, appending a suffix., Splits text into chunks within the limit, prioritizing paragraph and sentence bo, smart_split() (+12 more)

### Community 15 - "Mime-type helpers"
Cohesion: 0.13
Nodes (19): Interaction Engine, Sage Intelligence v3, clean_hashtags_if_needed(), post_to_bluesky(), post_to_mastodon(), post_to_threads(), Posts to Mastodon with Conditional Multi-Post Threading (The Weaver)., Helper to strip or format hashtags based on platform settings. (+11 more)
...
Cohesion: 0.50
Nodes (4): 1. Interactive Telegram Gateway & Approval Queue, 2. Screen Reader Multimodal Alt-Text, 3. Cultural Hashtag Alignment, 🚀 Page 17: Interactive Telegram Control, Alt Text, and Hashtag Management (v3.12.1)
...
- **Why does `Settings` connect `Feed configuration and dry-run diagnostics` to `Data Models & Pipeline Staging`, `Logger & Secret Redaction Engine`, `Global settings & config generation`, `Dashboard telemetry status migration`?**
  _High betweenness centrality (0.062) - this node is a cross-community bridge._
- **Why does `VanguardManager` connect `Feed Vanguard RSS audit resilience` to `Data Models & Pipeline Staging`, `Global settings & config generation`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `SafeLogger` connect `Logger & Secret Redaction Engine` to `Global settings & config generation`, `Feed configuration and dry-run diagnostics`, `Dashboard telemetry status migration`, `Seen interactions & smart text truncation`?**
