...
| Version | Supported |
| ------- | --------- |
| 3.18.x  | ✅ Active (Current) |
| 3.13.x  | ✅ Security patches only |
| 3.12.x  | ❌ Deprecated |
| 3.11.x  | ❌ Deprecated |
| < 3.11  | ❌ End of Life |
...
## Current Security Baseline (v3.18.3)
...
- **Dependency Exploits**: Core packages are pinned to specific versions via `pip-tools` (`requirements.txt` is generated by `pip-compile requirements.in`), preventing unvetted transitive dependency upgrades. Dependabot is configured with version constraints to prevent unresolvable upgrade loops. `cryptography` is constrained to `>=46.0.7,<47` resolving CVE-2026-39892.
