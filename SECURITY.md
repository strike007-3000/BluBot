...
- **Dependency Exploits**: Core packages are pinned to specific versions via `pip-tools` (`requirements.txt` is generated by `pip-compile requirements.in`), preventing unvetted transitive dependency upgrades. Dependabot is configured with version constraints to prevent unresolvable upgrade loops.
